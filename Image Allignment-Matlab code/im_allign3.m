...
%imwrite(cat(3,im(((2*row/3)+1):(row),:),sample_g,sample_r),'image4-corner.jpg');

fprintf('shift_r_row in Feature = %f\n', xrfinal);
  fprintf('shift_r_col in Feature= %f\n', yrfinal);
  fprintf('shift_g_row in Feature = %f\n', xgfinal);
  fprintf('shift_g_col in Feature = %f\n', ygfinal);
  
w = cat(3,im(((2*row/3)+1):(row),:),sample_g,sample_r);
end
 %toc